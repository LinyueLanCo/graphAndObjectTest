...
本文件夹存放了我们在项目重构与演进过程中绘制的**核心设计草图（Sketches）**。为了帮助你更直观地理解数据流动、职责边界以及底层的具体实现，本篇文档将重点结合这些草图的思路，详细讲解当前 C++ 代码的底层运行机制。
...
草图提出：预分配一个连续大数组作为对象池，维护活跃与空闲索引，并通过哈希表建立实体唯一 ID 到数组下标的常数时间映射。

### 💻 C++ 代码映射与实现
这一机制在 [EntityManager.h/.cpp](file:///c:/Users/lpy16/OneDrive/Desktop/c++/GraphAndObjTest/graphAndObjectTest/EntityManager.h) 中得以完整实现：

#### ① 对象池预分配与双索引列表
* **连续对象池**：`std::vector<Entity> entities`。在构造时直接预分配 `200` 个槽位：
  ```cpp
  entities.resize(200);
  ```
* **双索引划分**：
...
* **物理更新时的极速遍历**：更新逻辑时，只遍历 `activeIndices`，完全跳过空闲槽位，执行效率极高：
  ```cpp
...
每个实体在诞生时会被分配一个全局唯一的 `EntityID`。为了在 $O(1)$ 时间内定位实体而不需要全表遍历，我们使用了 `std::unordered_map`：
* **结构定义**：
  ```cpp
...
当玩家打碎宝箱生成金币时，我们绝对不能直接插入 `activeIndices`。我们采用**延迟生成缓存队列**：
...
草图提出：背景图层应该拥有自己的“视差系数（parallaxFactor）”和独立的位置变换逻辑。由背景层“自我更新”运行时坐标，最后再像普通世界物体一样丢给渲染器进行标准的相机坐标变换绘制。

### 💻 C++ 代码映射与实现
这一思想在 [Camera.h](file:///c:/Users/lpy16/OneDrive/Desktop/c++/GraphAndObjTest/graphAndObjectTest/Camera.h)、[BackgroundObject.h/.cpp](file:///c:/Users/lpy16/OneDrive/Desktop/c++/GraphAndObjTest/graphAndObjectTest/BackgroundObject.h) 中得以贯彻：
...
草图提出：背景、瓦片和角色都不要自己画图。它们在 Draw 阶段只做一件事——生成一个轻量的 `Sprite`（精灵单帧数据）并提交（submit）到一个统一的渲染队列中。队列对精灵按照 `zIndex` 进行排序，最后由 Renderer 统一、顺序地画在双缓冲内存中。

### 💻 C++ 代码映射与实现
这套高规格的渲染流由 [RenderQueue.h/.cpp](file:///c:/Users/lpy16/OneDrive/Desktop/c++/GraphAndObjTest/graphAndObjectTest/RenderQueue.h) 承载：
...
* **为什么使用稳定排序（`stable_sort`）？**
  ...
  `std::stable_sort` 保证了当 `zIndex` 相同且没有被我们手动指定谁遮挡谁时，**严格保留它们被提交进队列的先后物理顺序**，从而确保渲染表现极其稳定：
  ```cpp
...
在此过程中，我们还通过 `gCamera` 的屏幕可见矩形与 `sprite` 的世界中心包围盒进行碰撞相交判断，非可视区域内的精灵直接不予绘制，从而节省了底层绘制带宽， Debug 面板中显示的真实渲染数就是该裁剪统计后的精确数据。
...
### 💻 C++ 代码映射与实现
项目中的 UI 框架在 [UI.h/.cpp](file:///c:/Users/lpy16/OneDrive/Desktop/c++/GraphAndObjTest/graphAndObjectTest/UI.h) 与像素打字机对话框 [DialogueBox.cpp](file:///c:/Users/lpy16/OneDrive/Desktop/c++/GraphAndObjTest/graphAndObjectTest/DialogueBox.cpp) 中得到了深度体现：

#### ① 屏幕锚点对齐 (`UIAnchor`)
在创建 `UIElement` 时，不指定死坐标，而是指定 `UIAnchor`（如 `UI_BOTTOM_LEFT`、`UI_CENTER`）和偏置值 `marginX / Y`：
* 在初始化和屏幕大小调整时，调用 `makeUIBoxByAnchor`：根据窗口尺寸（1600x900）自动计算出对应的目标位置坐标 `targetX / targetY`。

#### ② 物理缓动与形变插值 (`MathUtils::smoothTo`)
UIElement 的真实位置 `x, y` 和宽高 `w, h` 并不会瞬间跳变，而是每帧在其 `update()` 中，向目标坐标和大小缓慢插值逼近，展现出极其 premium 的视觉动感：
```cpp
...
#### ③ 对话框文字排版换行与打字同步生长
为了在打字机逐字蹦出文字时，对话框能够“像长高一样往上顶起”，我们在 `DialogueBox` 中实现了以下高度计算公式：
...
* 每帧打出新字时，利用此高度公式计算当前文本所需的物理高度，并将其作为全新的 `targetH` 赋予 UI 元素：
  ...
  ```
  由于 `y` 坐标是对齐底部中心点的，当目标高度 `targetH` 变大时，其对齐后的目标 Y 坐标 `targetY` 会同步减少。于是，在 UI 的 `update()` 阶段，对话框不仅高度 `h` 变高，其位置 `y` 也会由于插值平滑往上滑，实现了**宛如气泡充气生长般的动态文字排版拉伸特效**。

---

## 💡 总结
这套代码架构的精髓在于**逻辑层与数据表现层的彻底解耦**。
草图是我们的灵感图纸，而最终演化出来的 C++ 代码，通过连续内存对象池、解耦的精灵提交队列、高度自治的背景视差计算和阻尼 UI 缓动，实现了一个高效、高规范、运行流畅且极其稳定的 2D 游戏底层框架。
